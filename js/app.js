...
(function () {
  ...
  function paywallSheet(featureName) {
    ...
    <div class="unlock-panel">
      ...
      <button class="btn btn-primary" id="unlockBtn">Unlock Full Access</button>
      <div style="height:10px"></div>
      <button class="btn btn-ghost" id="devPreviewBtn">Preview anyway (dev)</button>
    </div>`;
  }

  function wirePaywall(container) {
    const unlockBtn = container.querySelector("#unlockBtn");
    const devBtn = container.querySelector("#devPreviewBtn");
    if (unlockBtn) unlockBtn.addEventListener("click", startCheckout);
    if (devBtn) devBtn.addEventListener("click", () => {
      state.unlocked = true; save(); render();
    });
  }

  function startCheckout() {
    // Wired against worker.js /api/create-checkout-session once Stripe keys
    // are configured server-side. Falls back to a friendly notice in preview.
    fetch("/api/create-checkout-session", { method: "POST" })
      .then((r) => r.json())
      .then((d) => { if (d && d.url) location.href = d.url; else throw new Error("no url"); })
      .catch(() => {
        openModal(`
          <div style="text-align:center; padding: 10px 4px 4px;">
            <p class="card-sub" style="margin-bottom:18px;">Checkout isn't connected in this preview build yet. Once the Stripe worker is deployed, this button opens secure checkout.</p>
            <button class="btn btn-primary" id="closeCheckoutNotice">Got it</button>
          </div>`);
        document.getElementById("closeCheckoutNotice").addEventListener("click", closeModal);
      });
  }
